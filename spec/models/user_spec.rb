...
describe User do
  ...
  it "should reject invalid names" do
    names = %w[about contact juan_carlos]
    names.each do |name|
  ...
  describe "password validations" do
    it "should require a password" do
      no_pass_user = User.new(@attr.merge(
                         :password => "", :password_confirmation => ""))
      no_pass_user.should_not be_valid
    end

    it "should require a matching password confirmation" do
      inconsistent_passwords_user = User.new(@attr.merge( 
                                    :password_confirmation => "bar"))
      inconsistent_passwords_user.should_not be_valid
    end

    it "should reject short passwords" do
      short = "a" * 5
      short_password_user = User.new(@attr.merge(
  ...
  describe "password encryption" do
    ...
    describe "authenticate method" do
     ...
     it "should return nil for an email address with no user" do
        non_existentent_user = User.authenticate("nonexist@no.com", 
                                                  @attr[:password])
        non_existentent_user.should be_nil
     end
